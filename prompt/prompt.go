...
func GetFinishReasoningPrompt() string {
	return `如果你觉得当前已经解决用户的问题，或当前内容已经能够回答用户的问题，请直接返回[finish],除此之外什么都不返回
如果你只是完成了用户需求中的部分内容，你需要继续执行命令来观察才能完全解决用户的问题，请继续规划接下来要执行的命令`
}
...
如果用户的命令比较危险，需要写入数据, 你需要返回{"safe":false, "reason": "命令危险原因及可能会导致的后果的简短概括"} 
用户的操作系统是: %s
用户当前所处目录是: %s
用户当前的命令是: %v
你应当总是以JSON格式返回, 没有任何意外情况
你不能返回Markdown格式, 只返回JSON格式本身即可`, os, userDir, cmd)
}